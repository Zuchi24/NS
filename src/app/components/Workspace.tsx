...
export function Workspace() {
  ...
  const haveDuplicateIpAddresses = (
    a: Device,
    b: Device
  ) => {
  ...
  const isEndDevice = (device: Device) => {
    return (
  ...
  const getConnectionValidity = (
    connection: Connection
  ) => {
    const source = getDeviceById(
      connection.from
    );

    const target = getDeviceById(
      connection.to
    );
  ...
  const isDeviceConnectedToEndDevice = (
    ...
    return connections.some((connection) => {
      if (
        !getConnectionValidity(connection)
      ) {
        return false;
      }
